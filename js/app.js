...
function revealAndCheck(e) {
  if (e.target.getAttribute('class').includes('game-card')) {
    ...
    if (target.getAttribute('class').includes('hidden'))
      target.setAttribute(
        'class',
        target.getAttribute('class').replace(/hidden/gi, 'revealed')
      );
...
function addRandomIcons() {
  ...
  setTimeout(() => {
    for (let card of initCards) {
      ...
      card.setAttribute(
        'class',
        'game-card hidden ' + icon.getAttribute('class').substr(7)
      );
      hideCards(card);
    }
  }, 1000);
...
function hideCards(card) {
  if (card.getAttribute('class').includes('revealed')) {
    card.setAttribute(
      'class',
      card.getAttribute('class').replace(/revealed/gi, 'hidden')
    );
  ...
  if (card.getAttribute('class').includes('false')) {
    card.setAttribute(
      'class',
      card.getAttribute('class').replace(/false/gi, 'hidden')
    );
    card.firstElementChild.style.display = 'none';
  }
}
...
function checkMatched(target) {
  for (let card of initCards) {
    if (
      card !== target &&
      card.getAttribute('class') === target.getAttribute('class')
    ) {
      card.setAttribute(
        'class',
        card.getAttribute('class').replace(/revealed/gi, 'true')
      );
      target.setAttribute(
        'class',
        target.getAttribute('class').replace(/revealed/gi, 'true')
      );
      matches++;
      moves++;
      break;
...
function checkUnmatched(target) {
  for (let card of initCards) {
    if (
      card.getAttribute('class').includes('revealed') &&
      card.getAttribute('class') !== target.getAttribute('class')
    ) {
      navigator.vibrate(500);
      card.setAttribute(
        'class',
        card.getAttribute('class').replace(/revealed/gi, 'false')
      );
      target.setAttribute(
        'class',
        target.getAttribute('class').replace(/revealed/gi, 'false')
      );
      moves++;
      setTimeout(() => {
        hideCards(target);
